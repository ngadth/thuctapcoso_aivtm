...
class Painter():
    ...
    def __init__(self):
        Painter.gc_mode = 1
        Painter.cap = cv2.VideoCapture(0)
        Painter.cap.set(3, 1280)
        Painter.cap.set(4, 720)
    ...
    def build(self):
        ...
        #tao mot khung canvas de ve tren do
        canvas = np.zeros((720, 1280, 3), np.uint8)

        # xác định một điểm trước đó sẽ được sử dụng để vẽ một đường thẳng
        px, py = 0, 0
        # mau co ban dau la mau đỏ
        color = (255, 0, 0)
        #####
        brushSize = 5
        eraserSize = 20
        ...
        # xác định một bảng trắng để vẽ trên đó
        ...
        while Painter.cap.isOpened() and Painter.gc_mode :
            if coolingCounter:
                coolingCounter -= 1
                # print(coolingCounter)

            ret, frame = Painter.cap.read()
            if not ret:
                break
            frame = cv2.resize(frame, (1280, 720))
            frame = cv2.flip(frame, 1) #lật khung
